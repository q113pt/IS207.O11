...
<div class="products-container dark:!bg-slate-800">
    ...
    if (isset($_POST['sortOption']) && isset($_POST['loaisp'])) {
        ...
        <select name="sapxep" class="dark:text-slate-300 dark:bg-white/5" onchange="sortPL(' . $_POST['loaisp'] . ', this.value)">
    ...
    <option class="dark:text-slate-40 dark:bg-slate-800" value="lowhigh" ' . ($sortOption == 'lowhigh' ? 'selected' : '') . '>Từ thấp đến cao</option>
        ...
        */
        ...
        if ($sortOption === 'highlow') {
            ...
            $sql = "SELECT * FROM product WHERE  `Product category ID` = $loaisp";
        }
